...
async function getUsersWithPagination(page, perPage, search, sort) {
  try {
    if (typeof sort !== 'object') {
      // Melakukan pengecheckan sort merupakan objek atau bukan.
      throw new Error('Sort parameter must be an object');
    }

    // Mendapatkan user yang sudah dipagination dari repository.
    ...
    const totalPages = Math.ceil(users.total_pages / perPage);

    // Mengembalikan sesuai ketentuan di soal.
    return {
    ...
    // Ini untuk catch jika ada error
    throw error;
  }
}
